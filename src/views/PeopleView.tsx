...
export function PeopleView() {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 text-slate-400">
        <div className="glass-card p-8 text-center">
          ...
          <p className="text-shadow">No people tracked</p>
  ...
  return (
    <div className="space-y-4 pb-20">
      {/* Summary */}
      <div className={`glass-card p-4 ${homeCount > 0 ? 'glow-green' : ''}`}>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-slate-400">Currently at home</p>
          ...
          <div className={`w-12 h-12 rounded-full flex items-center justify-center ${homeCount > 0 ? 'bg-green-500/20' : 'glass-panel'}`}>
            <svg className={`w-6 h-6 ${homeCount > 0 ? 'text-green-400 drop-shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'text-slate-500'}`} viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="space-y-3">
        {sortedPeople.map(person => {
          ...
          return (
            <div key={person.entity_id} className={`glass-card p-4 transition-all duration-300 ${isHome ? 'glow-green bg-gradient-to-br from-green-500/10 to-emerald-500/5' : ''}`}>
              <div className="flex items-center gap-4">
                {/* Avatar */}
                <div className={`w-14 h-14 rounded-full flex items-center justify-center overflow-hidden ${isHome ? 'ring-2 ring-green-400 ring-offset-2 ring-offset-slate-900' : ''}`}>
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-white text-lg text-shadow">{name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`w-2 h-2 rounded-full ${statusColor} ${isHome ? 'shadow-[0_0_6px_rgba(34,197,94,0.6)]' : ''}`} />
                    <span className={`text-sm ${isHome ? 'text-green-400' : 'text-slate-400'}`}>
                      {status}
                    </span>
              ...
              {person.attributes.source && (
                <div className="mt-3 pt-3 border-t border-white/10 flex items-center gap-4 text-sm text-slate-500">
